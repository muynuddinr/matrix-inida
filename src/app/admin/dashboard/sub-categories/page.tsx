...
export default function SubCategoriesPage() {
  ...
  const handleStartEditSubCategory = (subcat: SubCategory) => {
    setSelectedSubCat(subcat);
    setFormData({
      name: subcat.name || '',
      category: categories.find((c) => c.id === subcat.category_id)?.name || '',
      slug: subcat.slug || '',
  ...
  const handleEditSubCategory = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedSubCat) {
      alert('No sub-category selected for editing');
      return;
    }
    if (!formData.name || !formData.category || !formData.slug || !formData.description) {
    ...
    try {
      ...
      const response = await fetch('/api/sub-categories/update', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const data = await response.json();
      setSubcategories(subcategories.map((s) => (s.id === selectedSubCat.id ? data.subCategory : s)));
  ...
  const handleDeleteSubCategory = async (subcat: SubCategory) => {
    if (!confirm(`Are you sure you want to delete "${subcat.name}"? This action cannot be undone.`)) {
      return;
    }

    try {
      const response = await fetch(`/api/sub-categories/${subcat.id}`, {
        method: 'DELETE',
      });
      ...
      alert(error instanceof Error ? error.message : 'Failed to delete sub-category');
    }
  };
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 space-y-2 border-t border-gray-200">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      if (selectedSubCat) handleStartEditSubCategory(selectedSubCat);
                    }}
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      if (selectedSubCat) handleDeleteSubCategory(selectedSubCat);
                    }}
    ...
    {showAddForm && (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={isEditing ? handleEditSubCategory : handleAddSubCategory} className="p-6 space-y-4">
            <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200">
              ...
              >
                {isSubmitting ? (isEditing ? 'Saving...' : 'Adding...') : (isEditing ? 'Save Changes' : 'Add Sub Category')}
              </button>
              <button
                type="button"
